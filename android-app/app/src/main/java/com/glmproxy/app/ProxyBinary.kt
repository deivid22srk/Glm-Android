...
object ProxyBinary {
    ...
    fun start(context: Context): Process {
        ...
        //
        // Preferred client: standalone-browser. When a captcha is needed,
        // the ProxyService log listener detects it and opens WebViewActivity
        // which polls /zcode/captcha/poll?client=standalone-browser.
        // The Go proxy retries the chat completion, and the next captcha
        // request goes to the standalone-browser client (the visible WebView).
        //
        // No background WebView — the visible WebView IS the captcha client.
        // This avoids the competing-consumer problem where a background
        // WebView would consume requests but fail to solve them.
        //
        ...
        // Increase captcha timeout from default 120s to 300s (5 min).
        // The default 2-minute timeout is too short for interactive
        // captchas (sliders, image picks) on mobile — by the time the
        // user sees the popup and solves it, the request has already
        // expired and the proxy has started a new one, causing a loop.
        env["ZCODE_CAPTCHA_BRIDGE_TIMEOUT_MS"] = "300000"
